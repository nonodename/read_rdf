...
SerdStatus serd_byte_source_open_source(SerdByteSource *source, SerdSource read_func, SerdStreamErrorFunc error_func,
...
static inline SERD_PURE_FUNC uint8_t serd_byte_source_peek(SerdByteSource *const source) {
	assert(source->prepared);
	return source->read_buf[source->read_head];
}

static inline SerdStatus serd_byte_source_advance(SerdByteSource *const source) {
	...
	if (source->from_stream) {
		...
		if (source->page_size > 1) {
			...
			if (!source->read_func(&source->read_byte, 1, 1, source->stream)) {
				source->eof = true;
				st = source->error_func(source->stream) ? SERD_ERR_UNKNOWN : SERD_FAILURE;
			}
		}
	} else if (!source->eof) {
