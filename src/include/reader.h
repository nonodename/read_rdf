...
#define SERD_STACK_ASSERT_TOP(reader, ref) assert(ref == reader->allocs[reader->n_allocs - 1])
#else
...
SerdStatus r_err_char(SerdReader *reader, const char *kind, int c);
...
SERD_PURE_FUNC size_t genid_size(const SerdReader *reader);
...
bool pop_last_node_char(SerdReader *reader, SerdNode *node);
...
SERD_NODISCARD static inline int eat_byte_safe(SerdReader *const reader, const int byte) {
	(void)byte;
...
SERD_NODISCARD static inline int eat_byte_check(SerdReader *const reader, const int byte) {
...
static inline SerdStatus eat_string(SerdReader *const reader, const char *const str, const unsigned n) {
...
static inline SerdStatus push_byte(SerdReader *const reader, const Ref ref, const int c) {
	assert(c >= 0);
...
static inline void push_bytes(SerdReader *const reader, const Ref ref, const uint8_t *const bytes, const unsigned len) {
