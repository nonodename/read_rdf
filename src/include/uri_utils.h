...
static inline bool chunk_equals(const SerdChunk *const a, const SerdChunk *const b) {
	return a->len == b->len && !strncmp((const char *)a->buf, (const char *)b->buf, a->len);
}

static inline size_t uri_path_len(const SerdURI *const uri) {
	return uri->path_base.len + uri->path.len;
}

static inline uint8_t uri_path_at(const SerdURI *const uri, const size_t i) {
	return (i < uri->path_base.len) ? uri->path_base.buf[i] : uri->path.buf[i - uri->path_base.len];
}
...
*/
static inline SERD_PURE_FUNC SlashIndexes uri_rooted_index(const SerdURI *const uri, const SerdURI *const root) {
	SlashIndexes indexes = {SIZE_MAX, SIZE_MAX};

	if (!root || !root->scheme.len || !chunk_equals(&root->scheme, &uri->scheme) ||
...
static inline SERD_PURE_FUNC bool uri_is_related(const SerdURI *const uri, const SerdURI *const root) {
	return uri_rooted_index(uri, root).shared != SIZE_MAX;
}

/// Return true iff `uri` is within the base of `root`
static inline SERD_PURE_FUNC bool uri_is_under(const SerdURI *const uri, const SerdURI *const root) {
	const SlashIndexes indexes = uri_rooted_index(uri, root);
	return indexes.shared && indexes.shared != SIZE_MAX && indexes.shared == indexes.root;
}
