...
static inline bool in_range(const int c, const int min, const int max) {
	return (c >= min && c <= max);
}

/// RFC2234: ALPHA ::= %x41-5A / %x61-7A ; A-Z / a-z
static inline bool is_alpha(const int c) {
...
static inline bool is_base64(const int c) {
	return is_alpha(c) || is_digit(c) || c == '+' || c == '/' || c == '=';
}

static inline bool is_windows_path(const uint8_t *path) {
	return is_alpha(path[0]) && (path[1] == ':' || path[1] == '|') && (path[2] == '/' || path[2] == '\\');
}

size_t serd_substrlen(const uint8_t *str, size_t len, size_t *n_bytes, SerdNodeFlags *flags);

static inline uint8_t hex_digit_value(const uint8_t c) {
	return (uint8_t)((c > '9') ? ((c & ~0x20) - 'A' + 10) : (c - '0'));
}

static inline char serd_to_upper(const char c) {
	return (char)(in_range(c, 'a', 'z') ? (c - 32) : c);
}

SERD_PURE_FUNC static inline int serd_strcasecmp(const char *s1, const char *s2) {
	while (*s1 && *s2) {
...
static inline uint8_t utf8_num_bytes(const uint8_t leading) {
...
static inline uint32_t parse_counted_utf8_char(const uint8_t *const utf8, const uint8_t size) {
...
static inline uint32_t parse_utf8_char(const uint8_t *const utf8, uint8_t *const size) {
