...
static inline SerdStack serd_stack_new(const size_t size) {
	SerdStack stack;
...
static inline bool serd_stack_is_empty(const SerdStack *const stack) {
	return stack->size <= SERD_STACK_BOTTOM;
}

static inline void serd_stack_free(SerdStack *const stack) {
	free(stack->buf);
	stack->buf = NULL;
	stack->buf_size = 0;
	stack->size = 0;
...
static inline void *serd_stack_push(SerdStack *const stack, const size_t n_bytes) {
...
static inline void serd_stack_pop(SerdStack *const stack, const size_t n_bytes) {
	assert(stack->size >= n_bytes);
	stack->size -= n_bytes;
}

static inline void *serd_stack_push_aligned(SerdStack *const stack, const size_t n_bytes, const size_t align) {
...
static inline void serd_stack_pop_aligned(SerdStack *const stack, const size_t n_bytes) {
