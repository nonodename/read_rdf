...
static inline SerdByteSink serd_byte_sink_new(SerdSink sink, void *const stream, const size_t block_size) {
...
static inline SerdStatus serd_byte_sink_flush(SerdByteSink *const bsink) {
...
static inline void serd_byte_sink_free(SerdByteSink *const bsink) {
	serd_byte_sink_flush(bsink);
	serd_free_aligned(bsink->buf);
	bsink->buf = NULL;
}

static inline size_t serd_byte_sink_write(const void *buf, size_t len, SerdByteSink *const bsink) {
	if (len == 0) {
		return 0;
	}
