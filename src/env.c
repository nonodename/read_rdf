...
SerdEnv *serd_env_new(const SerdNode *const base_uri) {
...
void serd_env_free(SerdEnv *const env) {
	if (!env) {
		return;
	}
...
const SerdNode *serd_env_get_base_uri(const SerdEnv *const env, SerdURI *const out) {
	assert(env);
...
SerdStatus serd_env_set_base_uri(SerdEnv *const env, const SerdNode *const uri) {
	assert(env);
	...
	SerdURI base_uri;
	SerdNode base_uri_node = serd_node_new_uri_from_node(uri, &env->base_uri, &base_uri);
...
SERD_PURE_FUNC static SerdPrefix *serd_env_find(const SerdEnv *const env, const uint8_t *const name,
...
static void serd_env_add(SerdEnv *const env, const SerdNode *const name, const SerdNode *const uri) {
	...
	if (prefix) {
		...
		SerdPrefix *const new_prefixes = (SerdPrefix *)realloc(env->prefixes, (++env->n_prefixes) * sizeof(SerdPrefix));
		if (new_prefixes) {
...
SerdStatus serd_env_set_prefix(SerdEnv *const env, const SerdNode *const name, const SerdNode *const uri) {
	assert(env);
	assert(name);
	assert(uri);
	...
	if (serd_uri_string_has_scheme(uri->buf)) {
		...
		SerdURI abs_uri;
		SerdNode abs_uri_node = serd_node_new_uri_from_node(uri, &env->base_uri, &abs_uri);
...
SerdStatus serd_env_set_prefix_from_strings(SerdEnv *const env, const uint8_t *const name, const uint8_t *const uri) {
	assert(env);
	assert(name);
	assert(uri);
...
bool serd_env_qualify(const SerdEnv *const env, const SerdNode *const uri, SerdNode *const prefix,
	...
	for (size_t i = 0; i < env->n_prefixes; ++i) {
		...
		if (uri->n_bytes >= prefix_uri->n_bytes) {
			if (!strncmp((const char *)uri->buf, (const char *)prefix_uri->buf, prefix_uri->n_bytes)) {
...
SerdStatus serd_env_expand(const SerdEnv *const env, const SerdNode *const curie, SerdChunk *const uri_prefix,
	...
	const uint8_t *const colon = (const uint8_t *)memchr(curie->buf, ':', curie->n_bytes + 1);
...
SerdNode serd_env_expand_node(const SerdEnv *const env, const SerdNode *const node) {
	assert(node);
...
void serd_env_foreach(const SerdEnv *const env, const SerdPrefixSink func, void *const handle) {
	assert(env);
	assert(func);
