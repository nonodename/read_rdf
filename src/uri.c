...
const uint8_t *serd_uri_to_path(const uint8_t *const uri) {
	assert(uri);
...
uint8_t *serd_file_uri_parse(const uint8_t *const uri, uint8_t **const hostname) {
	assert(uri);
...
SerdStatus serd_uri_parse(const uint8_t *const utf8, SerdURI *const out) {
	assert(utf8);
	assert(out);
...
*/
static const uint8_t *remove_dot_segments(const uint8_t *const path, const size_t len, size_t *const up) {
	*up = 0;
...
static void merge(SerdChunk *const base, SerdChunk *const path) {
	size_t up = 0;
...
void serd_uri_resolve(const SerdURI *const r, const SerdURI *const base, SerdURI *const t) {
	assert(r);
	assert(base);
	assert(t);
...
static size_t write_path_tail(const SerdSink sink, void *const stream, const SerdURI *const uri, const size_t i) {
...
static size_t write_rel_path(const SerdSink sink, void *const stream, const SerdURI *const uri,
...
static uint8_t serd_uri_path_starts_without_slash(const SerdURI *const uri) {
	return ((uri->path_base.len || uri->path.len) &&
	        ((!uri->path_base.len || uri->path_base.buf[0] != '/') && (!uri->path.len || uri->path.buf[0] != '/')));
}

/// See http://tools.ietf.org/html/rfc3986#section-5.3
size_t serd_uri_serialise_relative(const SerdURI *const uri, const SerdURI *const base, const SerdURI *const root,
                                   const SerdSink sink, void *const stream) {
	assert(uri);
	assert(sink);

	size_t len = 0;
	const bool relative = root ? uri_is_under(uri, root) : uri_is_related(uri, base);
	...
	if (!relative || (!len && base && base->query.buf)) {
		...
		if (uri->authority.buf) {
			...
			const bool authority_ends_with_slash =
			    (uri->authority.len > 0 && uri->authority.buf[uri->authority.len - 1] == '/');

			if (!authority_ends_with_slash && serd_uri_path_starts_without_slash(uri)) {
...
/// See http://tools.ietf.org/html/rfc3986#section-5.3
size_t serd_uri_serialise(const SerdURI *const uri, const SerdSink sink, void *const stream) {
	assert(uri);
	assert(sink);
