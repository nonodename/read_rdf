...
static size_t serd_uri_string_length(const SerdURI *const uri) {
...
static size_t string_sink(const void *const buf, const size_t len, void *const stream) {
...
SerdNode serd_node_from_string(const SerdType type, const uint8_t *const str) {
	if (!str) {
...
SerdNode serd_node_from_substring(const SerdType type, const uint8_t *const str, const size_t len) {
	if (!str) {
		return SERD_NODE_NULL;
	}
...
SerdNode serd_node_copy(const SerdNode *const node) {
	if (!node || !node->buf) {
		return SERD_NODE_NULL;
	}
...
bool serd_node_equals(const SerdNode *const a, const SerdNode *const b) {
	assert(a);
	assert(b);

	return (a == b) || (a->type == b->type && a->n_bytes == b->n_bytes && a->n_chars == b->n_chars &&
	                    ((a->buf == b->buf) || !memcmp((const char *)a->buf, (const char *)b->buf, a->n_bytes + 1)));
}

SerdNode serd_node_new_uri_from_node(const SerdNode *const uri_node, const SerdURI *const base, SerdURI *const out) {
	assert(uri_node);

	return (uri_node->type == SERD_URI && uri_node->buf) ? serd_node_new_uri_from_string(uri_node->buf, base, out)
	                                                     : SERD_NODE_NULL;
}

SerdNode serd_node_new_uri_from_string(const uint8_t *const str, const SerdURI *const base, SerdURI *const out) {
	if (!str || str[0] == '\0') {
...
static bool is_dir_sep(const char c) {
#ifdef _WIN32
	return c == '\\' || c == '/';
#else
	return c == '/';
...
SerdNode serd_node_new_file_uri(const uint8_t *const path, const uint8_t *const hostname, SerdURI *const out,
                                const bool escape) {
	assert(path);
...
SerdNode serd_node_new_uri(const SerdURI *const uri, const SerdURI *const base, SerdURI *const out) {
	assert(uri);
...
SerdNode serd_node_new_relative_uri(const SerdURI *const uri, const SerdURI *const base, const SerdURI *const root,
                                    SerdURI *const out) {
	assert(uri);
	...
	SerdNode node = {buf, 0, 0, 0, SERD_URI};
	uint8_t *ptr = buf;
	const size_t actual_len = serd_uri_serialise_relative(uri, base, root, string_sink, &ptr);
...
static unsigned serd_digits(const double abs) {
	const double lg = ceil(log10(floor(abs) + 1.0));
	return lg < 1.0 ? 1U : (unsigned)lg;
}

SerdNode serd_node_new_decimal(const double d, const unsigned frac_digits) {
	if (isnan(d) || isinf(d)) {
		return SERD_NODE_NULL;
	}
...
SerdNode serd_node_new_integer(const int64_t i) {
...
SerdNode serd_node_new_blob(const void *const buf, const size_t size, const bool wrap_lines) {
	assert(buf);
