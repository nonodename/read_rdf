...
_Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wmissing-declarations\"")
...
#define SERD_FALLTHROUGH
    ...
    static bool fancy_syntax(const SerdReader *const reader) {
...
static uint8_t read_HEX(SerdReader *const reader) {
...
static SerdStatus read_UCHAR(SerdReader *const reader, const Ref dest, uint32_t *const char_code) {
	...
	if (code < 0x00000080) {
		...
		r_err(reader, SERD_ERR_BAD_SYNTAX, "unicode character 0x%X out of range\n", code);
...
static SerdStatus read_ECHAR(SerdReader *const reader, const Ref dest, SerdNodeFlags *const flags) {
...
static SerdStatus bad_char(SerdReader *const reader, const char *const fmt, const uint8_t c) {
...
static SerdStatus read_utf8_bytes(SerdReader *const reader, uint8_t bytes[4], uint8_t *const size, const uint8_t c) {
...
static SerdStatus read_utf8_character(SerdReader *const reader, const Ref dest, const uint8_t c) {
	uint8_t size = 0U;
...
static SerdStatus read_utf8_code(SerdReader *const reader, const Ref dest, uint32_t *const code, const uint8_t c) {
	uint8_t size = 0U;
	uint8_t bytes[4] = {0, 0, 0, 0};
	SerdStatus st = read_utf8_bytes(reader, bytes, &size, c);
	if (st) {
...
static SerdStatus read_character(SerdReader *const reader, const Ref dest, SerdNodeFlags *const flags,
                                 const uint8_t c) {
	if (!(c & 0x80)) {
		if (c == 0xA || c == 0xD) {
			*flags |= SERD_HAS_NEWLINE;
...
// [10] comment ::= '#' ( [^#xA #xD] )*
static void read_comment(SerdReader *const reader) {
	skip_byte(reader, '#');
...
static bool read_ws(SerdReader *const reader) {
	const int c = peek_byte(reader);
	switch (c) {
	case 0x9:
	case 0xA:
...
static bool read_ws_star(SerdReader *const reader) {
...
static bool peek_delim(SerdReader *const reader, const uint8_t delim) {
	read_ws_star(reader);
	return peek_byte(reader) == delim;
}

static bool eat_delim(SerdReader *const reader, const uint8_t delim) {
...
static SerdStatus read_string_escape(SerdReader *const reader, const Ref ref, SerdNodeFlags *const flags) {
	SerdStatus st = SERD_SUCCESS;
	uint32_t code = 0;
	if ((st = read_ECHAR(reader, ref, flags)) && (st = read_UCHAR(reader, ref, &code))) {
...
static SerdStatus read_STRING_LITERAL_LONG(SerdReader *const reader, const Ref ref, SerdNodeFlags *const flags,
                                           const uint8_t q) {
	SerdStatus st = SERD_SUCCESS;
	...
	while (!(st && reader->strict)) {
		...
		if (c == '\\') {
			...
			st = read_character(reader, ref, flags, (uint8_t)eat_byte_safe(reader, c));
		} else {
...
static SerdStatus read_STRING_LITERAL(SerdReader *const reader, const Ref ref, SerdNodeFlags *const flags,
                                      const uint8_t q) {
	SerdStatus st = SERD_SUCCESS;
...
static SerdStatus read_String(SerdReader *const reader, const Ref node, SerdNodeFlags *const flags) {
	...
	if (!fancy_syntax(reader)) {
		return r_err(reader, SERD_ERR_BAD_SYNTAX, "syntax does not support long literals\n");
	}
...
static bool is_PN_CHARS_BASE(const uint32_t c) {
	return ((c >= 0x00C0 && c <= 0x00D6) || (c >= 0x00D8 && c <= 0x00F6) || (c >= 0x00F8 && c <= 0x02FF) ||
...
static SerdStatus read_PN_CHARS_BASE(SerdReader *const reader, const Ref dest) {
	uint32_t code = 0;
	const int c = peek_byte(reader);
	SerdStatus st = SERD_SUCCESS;
...
static bool is_PN_CHARS(const uint32_t c) {
	return (is_PN_CHARS_BASE(c) || c == 0xB7 || (c >= 0x0300 && c <= 0x036F) || (c >= 0x203F && c <= 0x2040));
}

static SerdStatus read_PN_CHARS(SerdReader *const reader, const Ref dest) {
	uint32_t code = 0;
	const int c = peek_byte(reader);
	SerdStatus st = SERD_SUCCESS;
...
static SerdStatus read_PERCENT(SerdReader *const reader, const Ref dest) {
...
static SerdStatus read_PN_LOCAL_ESC(SerdReader *const reader, const Ref dest) {
	skip_byte(reader, '\\');

	const int c = peek_byte(reader);
	return ((c == '!') || in_range(c, '#', '/') || (c == ';') || (c == '=') || (c == '?') || (c == '@') || (c == '_') ||
	        (c == '~'))
	           ? push_byte(reader, dest, eat_byte_safe(reader, c))
	           : r_err(reader, SERD_ERR_BAD_SYNTAX, "bad escape\n");
}

static SerdStatus read_PLX(SerdReader *const reader, const Ref dest) {
	const int c = peek_byte(reader);

	return (c == '%') ? read_PERCENT(reader, dest) : (c == '\\') ? read_PN_LOCAL_ESC(reader, dest) : SERD_FAILURE;
}

static SerdStatus read_PN_LOCAL(SerdReader *const reader, const Ref dest, bool *const ate_dot) {
...
static SerdStatus read_PN_PREFIX_tail(SerdReader *const reader, const Ref dest, bool *const ate_dot) {
	SerdStatus st = SERD_SUCCESS;
	bool trailing_unescaped_dot = false;
...
static SerdStatus read_PN_PREFIX(SerdReader *const reader, const Ref dest, bool *const ate_dot) {
	const SerdStatus st = read_PN_CHARS_BASE(reader, dest);

	return st ? st : read_PN_PREFIX_tail(reader, dest, ate_dot);
}

static SerdStatus read_LANGTAG(SerdReader *const reader, Ref *const dest) {
	int c = peek_byte(reader);
	if (!is_alpha(c)) {
...
static SerdStatus read_IRIREF_scheme(SerdReader *const reader, const Ref dest) {
	int c = peek_byte(reader);
	if (!is_alpha(c)) {
...
static SerdStatus read_IRIREF(SerdReader *const reader, Ref *const dest) {
	...
	*dest = push_node(reader, SERD_URI, "", 0);
	...
	while (st <= SERD_FAILURE) {
		...
		case '\\':
			...
			if (code == ' ' || code == '<' || code == '>') {
				*dest = pop_node(reader, *dest);
				return r_err(reader, SERD_ERR_BAD_SYNTAX, "invalid escaped IRI character U+%04X\n", code);
			}
			break;
		...
		default:
			if (c <= 0) {
				...
				st = r_err(reader, SERD_ERR_BAD_SYNTAX, "invalid IRI character (escape %%%02X)\n", (unsigned)c);
				if (!reader->strict) {
					st = SERD_FAILURE;
...
static SerdStatus read_PrefixedName(SerdReader *const reader, const Ref dest, bool *const ate_dot) {
	SerdStatus st = SERD_SUCCESS;
...
static SerdStatus read_0_9(SerdReader *const reader, const Ref str, const bool at_least_one) {
	unsigned count = 0;
	SerdStatus st = SERD_SUCCESS;
...
static SerdStatus read_number(SerdReader *const reader, Ref *const dest, Ref *const datatype, bool *const ate_dot) {
...
#define XSD_INTEGER NS_XSD "integer"
	...
	*dest = push_node(reader, SERD_LITERAL, "", 0);
	...
	if (c == 'e' || c == 'E') {
		...
		*datatype = push_node(reader, SERD_URI, XSD_DECIMAL, sizeof(XSD_DECIMAL) - 1);
	} else {
		*datatype = push_node(reader, SERD_URI, XSD_INTEGER, sizeof(XSD_INTEGER) - 1);
	}

	return SERD_SUCCESS;
}

static SerdStatus read_iri(SerdReader *const reader, Ref *const dest, bool *const ate_dot) {
	if (peek_byte(reader) == '<') {
...
static SerdStatus read_literal(SerdReader *const reader, Ref *const dest, Ref *const datatype, Ref *const lang,
                               SerdNodeFlags *const flags, bool *const ate_dot) {
	*dest = push_node(reader, SERD_LITERAL, "", 0);
...
static SerdStatus read_verb(SerdReader *const reader, Ref *const dest) {
	if (peek_byte(reader) == '<') {
		return read_IRIREF(reader, dest);
	}
...
static SerdStatus read_BLANK_NODE_LABEL(SerdReader *const reader, Ref *const dest, bool *const ate_dot) {
	skip_byte(reader, '_');
	if (!eat_byte_check(reader, ':')) {
		return SERD_ERR_BAD_SYNTAX;
	}
	...
	const Ref ref = *dest =
	    push_node(reader, SERD_BLANK, reader->bprefix ? (char *)reader->bprefix : "", reader->bprefix_len);
	...
	if (fancy_syntax(reader)) {
		if (is_digit(n->buf[reader->bprefix_len + 1])) {
			if ((n->buf[reader->bprefix_len]) == 'b') {
				...
				*dest = pop_node(reader, *dest);
				return r_err(reader, SERD_ERR_ID_CLASH, "found both 'b' and 'B' blank IDs, prefix required\n");
			}
		}
	}
...
static Ref read_blankName(SerdReader *const reader) {
...
static SerdStatus read_anon(SerdReader *const reader, ReadContext ctx, const bool subject, Ref *const dest) {
	skip_byte(reader, '[');
	...
	return st > SERD_FAILURE ? st : (eat_byte_check(reader, ']') == ']') ? SERD_SUCCESS : SERD_ERR_BAD_SYNTAX;
}

// Read a "named" object: a boolean literal or a prefixed name
static SerdStatus read_named_object(SerdReader *const reader, Ref *const dest, Ref *const datatype,
                                    bool *const ate_dot) {
...
static SerdStatus read_object(SerdReader *const reader, ReadContext *const ctx, const bool emit, bool *const ate_dot) {
#ifndef NDEBUG
...
static SerdStatus read_objectList(SerdReader *const reader, ReadContext ctx, bool *const ate_dot) {
	SerdStatus st = SERD_SUCCESS;
	TRY(st, read_object(reader, &ctx, true, ate_dot));
	if (!fancy_syntax(reader) && peek_delim(reader, ',')) {
		return r_err(reader, SERD_ERR_BAD_SYNTAX, "syntax does not support abbreviation\n");
	}
...
static SerdStatus read_predicateObjectList(SerdReader *const reader, ReadContext ctx, bool *const ate_dot) {
	SerdStatus st = SERD_SUCCESS;
...
static SerdStatus end_collection(SerdReader *const reader, const ReadContext ctx, const Ref n1, const Ref n2,
                                 const SerdStatus st) {
	pop_node(reader, n2);
	pop_node(reader, n1);
	*ctx.flags &= ~(unsigned)SERD_LIST_CONT;
...
static SerdStatus read_collection(SerdReader *const reader, ReadContext ctx, Ref *const dest) {
	SerdStatus st = SERD_SUCCESS;
	skip_byte(reader, '(');
...
static SerdStatus read_subject(SerdReader *const reader, const ReadContext ctx, Ref *const dest, int *const s_type) {
	SerdStatus st = SERD_SUCCESS;
	bool ate_dot = false;
...
static SerdStatus read_labelOrSubject(SerdReader *const reader, Ref *const dest) {
	bool ate_dot = false;
...
static SerdStatus read_triples(SerdReader *const reader, ReadContext ctx, bool *const ate_dot) {
	SerdStatus st = SERD_FAILURE;
	if (ctx.subject) {
		read_ws_star(reader);
		const int c = peek_byte(reader);
...
static SerdStatus read_base(SerdReader *const reader, const bool sparql, const bool token) {
	SerdStatus st = SERD_SUCCESS;
	if (token) {
...
static SerdStatus read_prefixID(SerdReader *const reader, const bool sparql, const bool token) {
	SerdStatus st = SERD_SUCCESS;
	if (token) {
	...
	if (reader->prefix_sink) {
		st = reader->prefix_sink(reader->handle, deref(reader, name), deref(reader, uri));
	}
...
static SerdStatus read_directive(SerdReader *const reader) {
...
static SerdStatus read_wrappedGraph(SerdReader *const reader, ReadContext *const ctx) {
	if (!eat_byte_check(reader, '{')) {
		return SERD_ERR_BAD_SYNTAX;
	}
...
static bool token_equals(SerdReader *const reader, const Ref ref, const char *const tok, const size_t n) {
...
SerdStatus read_n3_statement(SerdReader *const reader) {
#ifndef NDEBUG
	const size_t orig_stack_size = reader->stack.size;
#endif
	...
	case '@':
		if (!fancy_syntax(reader)) {
			return r_err(reader, SERD_ERR_BAD_SYNTAX, "syntax does not support directives\n");
		}
	...
	default:
		...
		if (token_equals(reader, ctx.subject, "base", 4)) {
			...
			ctx.graph = 0;
			read_ws_star(reader);
		} else if (token_equals(reader, ctx.subject, "true", 4) || token_equals(reader, ctx.subject, "false", 5)) {
			...
			if (ate_dot && (reader->strict || (s_type != '('))) {
				return r_err(reader, SERD_ERR_BAD_SYNTAX, "unexpected end of statement\n");
			}
...
SerdStatus serd_reader_skip_until_byte(SerdReader *const reader, const uint8_t byte) {
	assert(reader);
...
SerdStatus read_turtleTrigDoc(SerdReader *const reader) {
...
SerdStatus read_nquads_statement(SerdReader *const reader) {
	SerdStatus st = SERD_SUCCESS;
	SerdStatementFlags flags = 0;
	ReadContext ctx = {0, 0, 0, 0, 0, 0, &flags};
	bool ate_dot = false;
	...
	if ((st = read_subject(reader, ctx, &ctx.subject, &s_type)) || !read_ws_star(reader) ||
	    (st = read_IRIREF(reader, &ctx.predicate)) || !read_ws_star(reader) ||
...
SerdStatus read_nquadsDoc(SerdReader *const reader) {
	while (!reader->source.eof) {
