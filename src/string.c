...
const uint8_t *serd_strerror(const SerdStatus status) {
	switch (status) {
	case SERD_SUCCESS:
...
static void serd_update_flags(const uint8_t c, SerdNodeFlags *const flags) {
	if (c == '\r' || c == '\n') {
...
size_t serd_strlen(const uint8_t *const str, size_t *const n_bytes, SerdNodeFlags *const flags) {
	assert(str);
...
static double read_sign(const char **const sptr) {
	double sign = 1.0;
...
double serd_strtod(const char *const str, char **const endptr) {
	assert(str);
